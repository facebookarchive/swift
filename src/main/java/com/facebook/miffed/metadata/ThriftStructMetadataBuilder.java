...
import com.facebook.miffed.ThriftConstructor;
import com.facebook.miffed.ThriftField;
...
import static com.facebook.miffed.metadata.ReflectionHelper.getAllDeclaredFields;
import static com.facebook.miffed.metadata.ReflectionHelper.getAllDeclaredMethods;
import static com.facebook.miffed.metadata.ThriftStructMetadataBuilder.FieldMetadata.extractThriftFieldName;
import static com.facebook.miffed.metadata.ThriftStructMetadataBuilder.FieldMetadata.getOrExtractThriftFieldName;
import static com.facebook.miffed.metadata.ThriftStructMetadataBuilder.FieldMetadata.getThriftFieldId;
import static com.facebook.miffed.metadata.ThriftStructMetadataBuilder.FieldMetadata.getThriftFieldName;
import static com.facebook.miffed.metadata.ThriftStructMetadataBuilder.FieldMetadata.getThriftFieldProtocolType;
import static com.facebook.miffed.metadata.ThriftStructMetadataBuilder.FieldMetadata.getThriftFieldType;
...
public class ThriftStructMetadataBuilder<T>
{
    ...
    public ThriftStructMetadataBuilder(ThriftCatalog catalog, Class<T> structClass)
    {
        ...
        if (annotation != null) {
            ...
            problems.addError("Struct class [%s] does not have a @ThriftStruct annotation", structClass.getName());
            structName = structClass.getSimpleName();
        }
        ...
        for (Entry<Short, Collection<FieldMetadata>> entry : fieldsById.asMap().entrySet()) {
            ...
            if (id == null) {
                for (FieldMetadata field : fields) {
                    problems.addError("ThriftStruct %s field %s, does not have an id", structName, field.getName()); // todo bad message
                }
                continue;
            }
    ...
    private List<ThriftMethodInjection> toThriftMethodInjections(List<MethodInjection> methodInjections)
    {
        return Lists.transform(methodInjections, new Function<MethodInjection, ThriftMethodInjection>()
        {
            ...
            {
                return new ThriftMethodInjection(input.getMethod(), toThriftParameterInjections(input.getParameters()));
            }
        });
    }
    ...
    private List<ThriftParameterInjection> toThriftParameterInjections(List<ParameterInjection> parameters)
    {
        return Lists.transform(parameters, new Function<ParameterInjection, ThriftParameterInjection>()
        {
            ...
            {
                return new ThriftParameterInjection(input.getId(), input.getName(), input.getParameterIndex());
            }
        });
    }
    ...
    private List<ParameterInjection> getParameterInjections(String methodSignature, Annotation[][] parameterAnnotations, Type[] parameterTypes)
    {
        ...
        for (int parameterIndex = 0; parameterIndex < parameterAnnotations.length; parameterIndex++) {
            ...
            for (Annotation annotation : annotations) {
                ...
                ParameterInjection parameterInjection = new ParameterInjection(parameterIndex, thriftField, "arg" + parameterIndex, parameterType);
        ...
        static <T extends FieldMetadata> Function<T, ThriftType> getThriftFieldType(final ThriftCatalog catalog)
        {
            return new Function<T, ThriftType>()
            {
                ...
                public ThriftType apply(@Nullable T input)
                {
                    ...
                    ThriftProtocolFieldType protocolType = input.getProtocolType();
                    Preconditions.checkNotNull(protocolType, "protocolType is null for field %s", input.getName());
                    Type javaType = input.getJavaType();
    ...
    private static class ParameterInjection extends FieldMetadata
    {
        ...
        private ParameterInjection(int parameterIndex, ThriftField annotation, String extractedName, Type parameterJavaType)
        {
            ...
            this.parameterJavaType = Preconditions.checkNotNull(parameterJavaType, "parameterJavaType is null");

...
public class ThriftType
{
    ...
    private ThriftType(ThriftProtocolFieldType protocolType, ThriftType keyType, ThriftType valueType)
    {
        this.protocolType = protocolType;
    ...
    {
        Preconditions.checkState(keyType != null, "%s does not have a key", protocolType);
        return keyType;
    }

    public ThriftType getValueType()
    {
        Preconditions.checkState(keyType != null, "%s does not have a value", protocolType);
        return valueType;
    }

    public ThriftStructMetadata<?> getStructMetadata()
    {
        Preconditions.checkState(structMetadata != null, "%s does not have struct metadata", protocolType);

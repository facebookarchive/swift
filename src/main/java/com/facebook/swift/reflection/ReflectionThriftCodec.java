...
public class ReflectionThriftCodec implements ThriftCodec {
  ...
  public <T> T read(ThriftStructMetadata<T> metadata, TProtocol protocol) throws Exception {
    TField field;
    ...
    switch (type.getProtocolType()) {
      ...
      case MAP: {
        ...
        protocol.writeMapBegin(
          new TMap(
            keyType.getProtocolType().getType(),
            valueType.getProtocolType().getType(),
            map.size()
          )
        );
      ...
      case SET: {
        ...
        protocol.writeSetBegin(
          new TSet(
            elementType.getProtocolType().getType(),
            Iterables.size(list)
          )
        );
      ...
      case LIST: {
        ...
        protocol.writeListBegin(
          new TList(
            elementType.getProtocolType().getType(), Iterables.size(
            list
          )
          )
        );
